...
class COCOPipeline(Pipeline):
    ...
    def define_graph(self):
        ...
        # crop_begin, crop_size, bboxes, labels = self.prospective_crop(bboxes, labels) # cpu only
        # images = self.slice(images.gpu(), crop_begin, crop_size) # cpu and gpu

        h_flip, v_flip = self.coin_flip_h(), self.coin_flip_v()

        images = self.flip(images.gpu(), horizontal=h_flip, vertical=v_flip)
        bboxes = self.bbflip(bboxes.gpu(), horizontal=h_flip, vertical=v_flip)

        # images = self.water(images.gpu())
        # images = self.hsv(images.gpu()) not defined
        # images = self.sphere(images.gpu())
