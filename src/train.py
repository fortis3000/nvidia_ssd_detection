...
def train_loop(model, loss_func, epoch, optim, scheduler, train_dataloader, val_dataloader, encoder, iteration, logger, args, mean, std):
#     for nbatch, (img, _, img_size, bbox, label) in enumerate(train_dataloader):
    for nbatch, data in enumerate(train_dataloader):
        ...
        if args.local_rank == 0:
            last_lr = scheduler.get_lr() if float(torch.__version__[:3]) < 1.4 else scheduler.get_last_lr()
            logger.update_iter(epoch, iteration, loss.item(), last_lr)
