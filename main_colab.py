...
import torch
import numpy as np
from torch.optim.lr_scheduler import MultiStepLR, CosineAnnealingLR, CosineAnnealingWarmRestarts, CyclicLR
...
from src.data import TRAIN_SIZE
...
def make_parser():
    ...
    parser.add_argument('--evaluation', nargs='+', type=list, default=[10,20,25,30,35,40,45,50] + \
                                                                        list(range(55,2005,5)),
                        help='epochs at which to evaluate')
    parser.add_argument('--multistep', nargs='+', type=list, default=list(range(10, 510, 10)),
...
def train(train_loop_func, logger, args):
    ...
    optimizer = torch.optim.AdamW(tencent_trick(ssd300), lr=args.learning_rate, betas=(0.8, 0.999), eps=1e-08, weight_decay=0.01, amsgrad=True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # scheduler.step()
        iteration = train_loop_func(ssd300, loss_func, epoch, optimizer, scheduler, train_loader, val_dataloader, encoder, iteration,
...
if __name__ == "__main__":
    ...
    f = open(f'./models/outputs_colab.txt', 'w')
    sys.stdout = f
